...
async function ensureDriveFolder() {
  console.log('[SW] ensureDriveFolder called - using hardcoded folder ID');
  
  // Use hardcoded folder ID
  driveFolderId = HARDCODED_CONFIG.DRIVE_FOLDER_ID;
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log('[SW] Hardcoded folder verified:', folder.name, 'ID:', driveFolderId);
      if (!folder.trashed) {
    ...
    throw new Error('Hardcoded folder not accessible or trashed');
  } catch (e) {
...
async function ensureSheet() {
  console.log('[SW] ensureSheet called - using hardcoded sheet ID');
  
  // Use hardcoded sheet ID
  sheetId = HARDCODED_CONFIG.SHEET_ID;
  
  // Validate the spreadsheet exists and is accessible
  try {
    const response = await fetch(
      `${GOOGLE_APIS.SHEETS}/${sheetId}?fields=spreadsheetId,properties.title,sheets.properties`,
      { headers: { Authorization: `Bearer ${authToken}` } }
    );
    
    if (response.ok) {
      const sheet = await response.json();
      console.log('[SW] Hardcoded spreadsheet verified:', sheet.properties.title);
      ...
      if (!hasLogSheet) {
        console.log('[SW] Creating TikTok Download Log sheet tab in hardcoded spreadsheet');
    ...
    throw new Error('Hardcoded spreadsheet not accessible');
  } catch (e) {
    console.error('[SW] Hardcoded spreadsheet validation failed:', e.message);
    throw new Error('Cannot access the configured Google Sheet. Please contact the administrator.');
  }
}
